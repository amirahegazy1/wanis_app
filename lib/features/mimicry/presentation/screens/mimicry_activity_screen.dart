...
class _MimicryActivityScreenState extends State<MimicryActivityScreen> {
  ...
  Future<void> _startAnalysis() async {
    ...
    _cam.controller!.startImageStream((img) async {
      fc++;
      if (fc % 3 != 0 || _processing || _phase != _Phase.analyzing) return;
      _processing = true;
      final kps = await _pose.detectPose(img, sensorOrientation: _cam.sensorOrientation);
      if (kps != null && mounted) {
        _totalFrames++;
  ...
  bool _isWaving(List<List<double>> kps) {
    ...
    // Wrist raised above shoulder level (y smaller = higher); 0.03 threshold is
    // forgiving enough for children while still requiring a clear upward wave.
    return (lW[2] > 0.2 && lW[0] < sY - 0.03) || (rW[2] > 0.2 && rW[0] < sY - 0.03);
  }

  bool _isClapping(List<List<double>> kps) {
    final lW = kps[9], rW = kps[10];
    if (lW[2] < 0.2 || rW[2] < 0.2) return false;
    // Both wrists close horizontally (clap) AND at roughly the same height.
    final xDist = (lW[1] - rW[1]).abs();
    final yDist = (lW[0] - rW[0]).abs();
    return xDist < 0.20 && yDist < 0.15;
  }
  ...
  bool _isHandOnHead(List<List<double>> kps) {
    ...
    final headX = nose[1];
    // Wrist at or above nose level AND horizontally centered on the head.
    // The x-proximity (0.20) distinguishes this from a side-wave.
    bool lH = lW[2] > 0.2 && lW[0] < nose[0] + 0.05 && (lW[1] - headX).abs() < 0.20;
    bool rH = rW[2] > 0.2 && rW[0] < nose[0] + 0.05 && (rW[1] - headX).abs() < 0.20;
    return lH || rH;
  }
  ...
  bool _isStirring(List<List<double>> kps) {
    ...
    final sY = (lS[0] + rS[0]) / 2;
    final bodyX = (lS[1] + rS[1]) / 2;
    // Hand at torso level (between shoulders and below) AND near body center
    // horizontally — consistent with a stirring / cooking motion.
    bool lM = lW[2] > 0.2 && lW[0] > sY - 0.1 && lW[0] < sY + 0.3 && (lW[1] - bodyX).abs() < 0.25;
    bool rM = rW[2] > 0.2 && rW[0] > sY - 0.1 && rW[0] < sY + 0.3 && (rW[1] - bodyX).abs() < 0.25;
    return lM || rM;
  }
  ...
  bool _isCombingHair(List<List<double>> kps) {
    ...
    final sY = (lS[0] + rS[0]) / 2;
    final headX = nose[1];
    // Wrist above shoulders, near head height, and close to the head — the
    // wider x-range (0.30) accommodates combing to the side of the head.
    bool lC = lW[2] > 0.2 && lW[0] < sY && lW[0] < headY + 0.1 && (lW[1] - headX).abs() < 0.30;
    bool rC = rW[2] > 0.2 && rW[0] < sY && rW[0] < headY + 0.1 && (rW[1] - headX).abs() < 0.30;
    return lC || rC;
  }
  ...
  bool _isThrowingBall(List<List<double>> kps) {
    ...
    if (lS[2] < 0.3 && rS[2] < 0.3) return false;
    final sY = (lS[0] + rS[0]) / 2;
    // Throwing pose: wrist above shoulders, wrist above elbow (arm cocked),
    // elbow near shoulder level. Relaxed elbow threshold (0.15) for children.
    bool lT = lW[2] > 0.2 && lEl[2] > 0.2 && lW[0] < sY && lW[0] < lEl[0] && lEl[0] < sY + 0.15;
    bool rT = rW[2] > 0.2 && rEl[2] > 0.2 && rW[0] < sY && rW[0] < rEl[0] && rEl[0] < sY + 0.15;
    return lT || rT;
  }

  bool _isStraighteningHands(List<List<double>> kps) {
    final lW = kps[9], rW = kps[10], lEl = kps[7], rEl = kps[8], lS = kps[5], rS = kps[6];
    // Only require shoulders + wrists; elbows are optional (often occluded on
    // small children or low-res cameras).
    if (lS[2] < 0.3 || rS[2] < 0.3 || lW[2] < 0.15 || rW[2] < 0.15) return false;
    final sY = (lS[0] + rS[0]) / 2;
    // Both wrists at shoulder height and spread far apart (T-pose).
    final wristsAtHeight = (lW[0] - sY).abs() < 0.20 && (rW[0] - sY).abs() < 0.20;
    final wristsSpread = (lW[1] - rW[1]).abs() > 0.30;
    // Elbows roughly at shoulder height if visible, otherwise skip check.
    bool elbowsOk = true;
    if (lEl[2] > 0.2 && rEl[2] > 0.2) {
      elbowsOk = (lEl[0] - sY).abs() < 0.20 && (rEl[0] - sY).abs() < 0.20;
    }
    return wristsAtHeight && wristsSpread && elbowsOk;
  }
  ...
  bool _isCupStacking(List<List<double>> kps) {
    ...
    // Wrist between shoulders and hips, near body center — stacking motion.
    bool lSt = lW[2] > 0.2 && lW[0] > sY && lW[0] < hY && (lW[1] - bC).abs() < 0.35;
    bool rSt = rW[2] > 0.2 && rW[0] > sY && rW[0] < hY && (rW[1] - bC).abs() < 0.35;
    bool both = lSt && rSt && (lW[1] - rW[1]).abs() < 0.30;
    return both || (lSt && lW[2] > 0.3) || (rSt && rW[2] > 0.3);
  }
}
