...
class _OnboardingSignUpScreenState extends State<OnboardingSignUpScreen> {
  ...
  Future<void> _runAuth(Future<dynamic> Function() action) async {
    debugPrint('[SignUp] _runAuth START');
    setState(() {
      _isLoading = true;
      _errorMessage = null;
    });
    try {
      debugPrint('[SignUp] Calling auth action...');
      final result = await action().timeout(const Duration(seconds: 15));
      debugPrint('[SignUp] Auth action returned: ${result != null ? "success" : "null"}');
      ...
      if (!mounted) return;
      debugPrint('[SignUp] Navigating to AddChildScreen...');
      ...
      if (!mounted) return;
      setState(() => _errorMessage = 'انتهى وقت الطلب. يرجى التحقق من اتصالك بالإنترنت');
