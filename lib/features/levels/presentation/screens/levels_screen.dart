...
class _LevelsScreenState extends State<LevelsScreen> {
  ...
  Future<void> _fetchActiveDays() async {
    ...
    if (user != null) {
      try {
        final parent = await _firestoreService.getParentUser(user.uid);
        if (parent != null && mounted) {
          setState(() {
            _activeDays = parent.activeDays;
          });
        }
      } catch (e) {
        debugPrint('Error fetching activeDays: $e');
      }
    }
    if (mounted) {
      setState(() {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            _isLoadingDays ? 'جاري التحميل...' : 'المتبقي: ${(21 - _activeDays).clamp(0, 21)} يوم',
            style: _cairo(
              fontSize: 14,
              fontWeight: FontWeight.w700,
              color: _darkBlue,
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              value: (_activeDays / 21).clamp(0.0, 1.0),
              minHeight: 10,
              backgroundColor: const Color(0xFFE1E2E9),
              valueColor: const AlwaysStoppedAnimation<Color>(Color(0xFF2976C7)),
            ),
          ),
          const SizedBox(height: 16),
          ...
          if (_activeDays >= 21)
            GestureDetector(
              onTap: () {
                // Future implementation: Navigate to Post Test
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('سيتم فتح الاختبار قريباً!', style: _cairo())),
                );
              },
              child: Container(
  ...
  void _openParentPanel(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const _ParentPanelScreen()),
    );
  }
}
...
class _ParentPanelScreen extends StatelessWidget {
  const _ParentPanelScreen();
  ...
  /// Show the Math Gate, then navigate to [destination]
